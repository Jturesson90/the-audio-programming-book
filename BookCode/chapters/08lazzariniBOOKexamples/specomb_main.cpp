...
/** speccomb main program

\b usage: speccomb infile outfile dur(s) \n
\n
...
main(int argc, char **argv){
  ...
  float delay, fr = sr/hopsize;
  for(i=0, delay=.005f; i < outsize; i += fftsize,
	delay= (float)(.0055 + 0.005*cos((i/fftsize)*twopi*0.1/fr))) 
    specomb(&spec[i], &spec[i], .8f, delay, .7, 1024, sr);
