...
\b usage: pvmorph infile1 infile2 outfile dur(s) \n
\n
infile1: input1 (start sound) filename \n
infile2: input2 (final sounds) filename \n
...
main(int argc, char **argv){
  ...
  outsize = pva(in, win, spec, dur, 
		fftsize, hopsize,  sr);

  pva(in2, win, spec2, 
      dur, fftsize, hopsize, sr);

  for(i=0; i < specsize; i+=fftsize) {
    pvmorph(&spec[i], &spec2[i], &spec[i], 
	    (float)i/outsize,  (float)i/outsize,
	    1024, (float) sr);
  }

  dur = pvs(spec, win, out, 
	    specsize,  fftsize, hopsize, sr);
